document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadProjetos = async (ano) => {
    projetoContainer.innerHTML =
      '<p style="text-align: center;">Carregando projetos...</p>';
    try {
      ...
      if (projetos.length === 0) {
        projetoContainer.innerHTML =
          '<p style="text-align: center;">Nenhum projeto encontrado para este ano.</p>';
        return;
      }
      ...
      projetos.forEach((projeto) => {
        ...
        card.innerHTML = `
                    ...
                    <a href="${projeto.link}" target="_blank">Acessar Projeto X</a>
      ...
      projetoContainer.innerHTML =
        '<p style="text-align: center; color: red;">Não foi possível carregar os dados dos projetos.</p>';
    }
  };
  ...
  const handleReaction = async (event) => {
    ...
    try {
      ...
      document.getElementById(`gostei-count-${projectId}`).textContent =
        updatedProject.reactions.gostei;
      document.getElementById(`amei-count-${projectId}`).textContent =
        updatedProject.reactions.amei;
      document.getElementById(`susto-count-${projectId}`).textContent =
        updatedProject.reactions.susto;
